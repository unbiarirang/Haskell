...
-- | Buggy Binomial Heap implementation 2
...
link2 :: (Ord a) => Node a -> Node a -> Node a
link2 t1 t2 = if value t1 > value t2
...
ins2 :: (Ord a) => Node a -> BHeap a -> BHeap a
ins2 t (BHeap []) = BHeap [t]
ins2 t (BHeap (tp:ts)) = if rank t < rank tp
                        then BHeap (t:tp:ts)
                        else ins2 (link2 t tp) (BHeap ts)

-- | Correct
unwrap2 :: BHeap a -> [Node a]
unwrap2 (BHeap x) = x
...
instance PriorityQueue BHeap where
  ...
  insert x = ins2 Node { value=x, rank=0, children=[] }
  -- | Wrong due to wrong @link@ and @ins@
  meld (BHeap []) ts = ts
  meld ts (BHeap []) = ts
  meld (BHeap (t1:ts1)) (BHeap (t2:ts2))
    | rank t1 < rank t2 = BHeap (t1:unwrap2 (meld (BHeap ts1) (BHeap (t2:ts2))))
    | rank t2 < rank t1 = BHeap (t2:unwrap2 (meld (BHeap (t1:ts1)) (BHeap ts2)))
    | otherwise = ins2 (link2 t1 t2) (meld (BHeap ts1) (BHeap ts2))
  -- | Correct
